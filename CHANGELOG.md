...
- Windows：文件列表在 Shell 延迟暴露为 `file:///...` 文本或位图/DIB 时，会保留并恢复 `CF_HDROP`；图片文件不会再被误同步为普通图片而覆盖本地文件粘贴
- Windows：保留以点开头的文件名（如 `.cargo-artifact-lock`、`.env`），仅移除 Windows 不允许的尾随点
- 设备状态：界面以实际活动会话为准，已建立连接不会再被陈旧元数据展示成“自动重连中”
- 窗口行为：`—` 最小化到任务栏并可 Alt+Tab 切换；`×` 隐藏到托盘；托盘“退出”才真正结束程序。托盘启动后恢复窗口同样保留任务栏行为

### 测试

- 新增文件 URI / 位图中间态、点文件名传输和连接状态快照的回归单元测试
- `cargo test --lib`、Clippy 与 release 构建通过

## 0.1.43 — 2026-07-24

### 修复

- Windows：接收同步文件后，Shell 偶发暴露的 `file:///...` URI 文本不会再覆盖 `CF_HDROP` 文件列表并被再次同步；本机资源管理器可继续粘贴收到的文件
- 历史记录中的文件/文件夹“复制”恢复写入本地文件列表剪贴板，不再错误地当作远端同步写入并留下抑制状态

### 测试

- 全量本地单元、协议、Hub 集成测试与 Windows VM 实机剪贴板冒烟均通过

## 0.1.42 — 2026-07-24

### 修复

- Windows：从资源管理器复制 PNG/JPG 等图片文件时，接收端始终恢复为 `CF_HDROP` 文件列表，不再误写为位图，因而可在文件管理器中以文件形式粘贴
- Windows：原生 `CF_HDROP` 写入会重试暂时的剪贴板占用；同步抑制与内容指纹按文本、图片、文件三种语义分域，避免相同 PNG 字节的位图与文件复制互相吞掉
- 冒烟：停止所有历史 OhMyCopy 进程名后再部署，避免旧版本占用 3721 端口而让测试误测旧二进制；冒烟会构建实际应用二进制并额外验证图片文件的 VM 文件列表粘贴
